...
// Language support is now centralized globally per Test schema
// Deprecated: Per-test language selection removed
...
export function TestDetailClient({ testId }: TestDetailClientProps) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (
        testRes.status === "fulfilled" &&
        testRes.value.success &&
        testRes.value.data
      ) {
      ...
      if (
        ...
        inviteRes.value.success &&
        inviteRes.value.data
      ) {
  ...
  const handleDelete = async () => {
    ...
    if (
      !confirm(
        "This will delete the assessment and all related data. Continue?",
      )
    ) {
      return;
    }
          ...
          <motion.div variants={fadeUp}>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Edit core assessment properties
                </CardDescription>
              </CardHeader>
          ...
          <motion.div variants={fadeUp}>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Visible to candidates when not hidden
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
