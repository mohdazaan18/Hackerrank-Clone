...
export default function InvitePage({ params }: InvitePageProps) {
  ...
  useEffect(() => {
    ...
    const doLogin = async () => {
      ...
      setStatus("loading");
      try {
        // Clear any existing auth first to ensure clean candidate login
        await logout();
        ...
        if (err instanceof AxiosError && err.response?.data?.error) {
          ...
          if (apiErr.includes("complete a test") || apiErr.includes("Submission Finalized")) {
    ...
    doLogin();
  }, [token, candidateLogin, router, hasAttempted, authLoading, logout]);
